//
// <auto-generated>
// Hetzner Cloud API Client - Auto-generated from OpenAPI specification
//
// Generated by: ApiGenerator
// Source: specs/cloud.spec.json
// API: Hetzner Cloud API
// Version: 1.0.0
//
// Changes to this file may cause incorrect behavior and will be lost if
// the code is regenerated.
// </auto-generated>
//
using System;
using System.Collections.Generic;
using System.ComponentModel.DataAnnotations;
using System.Text.Json.Serialization;

#nullable enable

namespace Apigen.Hetzner.Cloud.Models;

/// <summary>
/// TargetTypeLabelSelectorTargets from OpenAPI schema.
/// Schema path: #/components/schemas/TargetTypeLabelSelectorTargets
/// </summary>
public partial class TargetTypeLabelSelectorTargets
{
  /// <summary>
  /// Type of the resource. Here always &quot;server&quot;.
  /// </summary>
  [Required]
  [System.Text.Json.Serialization.JsonPropertyName("type")]
  public string? Type { get; set; }

  /// <summary>
  /// Server where the traffic should be routed to. Only present for target type &quot;server&quot;.
  /// </summary>
  [Required]
  [System.Text.Json.Serialization.JsonPropertyName("server")]
  public TargetTypeServerServer Server { get; set; } = null!;

  /// <summary>
  /// List of health statuses of the services on this target. Only present for target types &quot;server&quot; and &quot;ip&quot;.
  /// </summary>
  [Required]
  [System.Text.Json.Serialization.JsonPropertyName("health_status")]
  public List<TargetTypeServerHealthStatus>? HealthStatus { get; set; }

  /// <summary>
  /// Use the private network IP instead of the public IP. Only present for target types &quot;server&quot; and &quot;label_selector&quot;.
  /// </summary>
  [Required]
  [System.Text.Json.Serialization.JsonPropertyName("use_private_ip")]
  public bool UsePrivateIp { get; set; }
}
